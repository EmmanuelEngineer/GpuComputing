...
number_of_iterations=1 #number of times a setting is tested
...
maximum_power=14 # Maximum n for matrices 2^n*2^n
block_dimension=8
minimum_power=5 # Minimum n for matrices 2^n*2^n 
#######################################
...
opt_num=$starting_optimizations
block_dimensions=(16 32 64)

until [ $opt_num -gt 3 ]; do
    dtype=0
    #0 is integers, 1 is float, 2 is double
    until [ $dtype -gt 2 ]; do
        for block_dimension in "${block_dimensions[@]}"; do
            make clean
            make DTYPE=$dtype OPT_NUM=$opt_num TILE_DIM=$block_dimension
            num=$minimum_power
            until [ $num -gt $up_to ]; do
                time=1
                until [ $time -gt $num_iterations ]; do
                    echo "$num\_$time\_$dtype\_$opt_num\_$block_dimension"
                    bin/runnable_$dtype $num &>"deliverable_runs/$num-$time-$dtype-$opt_num-$block_dimension.txt"
                    time=$(($time+1))
                done
                num=$(($num+1))
            done
